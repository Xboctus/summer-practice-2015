            ...
            if (user.isActive())
                for (Event event:user.getEvents())
                    if (event.getTime().getTime() >= curTime)
                        events.add(new ActiveEvent(user.getName(), event.getTime(), event.getDescription()));
        Collections.sort(events);

        for (int i = 0; i < events.size(); i++) {
            if (i == 0)
                Thread.sleep(events.get(i).getTime() - System.currentTimeMillis());
            else
                Thread.sleep(events.get(i).getTime() - events.get(i - 1).getTime());
            events.get(i).show();
        }
    }
}